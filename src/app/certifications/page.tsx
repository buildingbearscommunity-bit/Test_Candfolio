...
import { Search, ChevronRight, Filter, Clock, Award } from 'lucide-react';
import { useCertificationCards } from '@/lib/sheetExams';
import BrandLogoIcon from '@/components/BrandLogoIcon';

export default function CertificationsPage() {
  const { certCards, loading, error } = useCertificationCards();
  const [searchQuery, setSearchQuery] = useState('');

  // Filter by search only (categories come from sheet; no hardcoded list)
  const filtered = certCards.filter((card) =>
  ...
  return (
    <div className="space-y-10 py-4">
      ...
      <div className="space-y-2">
        <h1 className="text-3xl font-bold tracking-tight text-zinc-900">Certifications</h1>
        <p className="text-sm text-zinc-500 max-w-xl leading-relaxed">
          Choose a path to test your knowledge with timed certification exams, practice interactive mock interviews, and earn verifiable career credentials.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center bg-white p-4 rounded-xl border border-zinc-100" style={{ boxShadow: 'var(--shadow-card)' }}>
        ...
        <span className="text-xs text-zinc-400 font-medium flex items-center gap-1.5 flex-shrink-0">
          ...
          {loading ? 'Loading…' : `${certCards.length} certifications`}
        </span>
      </div>
      ...
      {error && !loading && (
        <div className="text-center py-16 bg-white border border-dashed border-zinc-200 rounded-xl space-y-3">
          <BrandLogoIcon className="h-8 w-8 mx-auto opacity-45" />
          <h3 className="text-sm font-semibold text-zinc-700">Could not load certifications</h3>
      ...
      {!loading && !error && (
        filtered.length > 0 ? (
            ...
            {filtered.map((card) => (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-4 mb-5">
                  <div className="p-3 bg-accent-light text-accent rounded-lg group-hover:bg-accent/15 transition-colors duration-150">
                    <Award className="h-6 w-6 stroke-[1.5]" />
                  </div>
                  <span className="badge badge-neutral mt-0.5">Certification</span>
                </div>

                {/* Title */}
                <h3 className="text-base font-semibold text-zinc-950 mb-1.5 group-hover:text-accent transition-colors duration-150 leading-snug">
                  {card.title}
                </h3>

                {/* Meta: questions · duration */}
                <p className="text-xs text-zinc-400 font-normal mb-6">
                  {card.totalQuestions > 0 ? `${card.totalQuestions} questions` : 'Questions coming soon'} · {card.durationMins} mins
                </p>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-zinc-50 text-zinc-400 text-[11px] font-medium">
                  <span className="flex items-center space-x-1">
                    ...
                    <span>Pass: {card.passThreshold}%</span>
                  </span>
          ...
          <div className="text-center py-16 bg-white border border-dashed border-zinc-200 rounded-xl">
            ...
            <h3 className="text-sm font-semibold text-zinc-700">No certifications found</h3>
